...
int find_msindic(int *indic,SVDRec SVD_Asym, double alpha,int n)
{
	int d = SVD_Asym->d ;
	int i,k;
	int *s2;
	s2 = malloc(d*sizeof(int));
	double sum;	
...
//	printf("%f\n",alpha);
	...
	for(i=s1+1; i<d; ++i)
	{
		...
		if( svdAbsolute( SVD_Asym->S[s1] - SVD_Asym->S[i] - 2*alpha ) < 1e-3 )
		{
			s2[s2count] = i;
			++s2count;
		}
		else if( svdAbsolute( SVD_Asym->S[s1] + SVD_Asym->S[i] - 2*alpha ) < 1e-3 )
		{
			s2[s2count] = i;
			++s2count;
		}
...
//	printf("s1 : %d s2count : %d\n",s1,s2count);
	...
	for(i=0; i<s2count; ++i)
	{
		sum = 0;
		for(k=0; k<n; ++k)
			sum += SVD_Asym->Vt->value[s1][k] * SVD_Asym->Vt->value[s2[i]][k] ;
		
		for(k=n; k<SVD_Asym->Vt->cols; ++k)
			sum += -SVD_Asym->Vt->value[s1][k] * SVD_Asym->Vt->value[s2[i]][k] ;
		
		if( svdAbsolute(sum) > 1e-1 )
		{
			indic[s2[i]] = 0;
			return(s1) ;
		}
...
/************************************************************************
 * For numerical stability, construct Asym and do svdLAS2(Asym,2*r).
 * Asym = [ a*I A;
 *           A  a*I;];
************************************************************************/
SVDRec svdLAS2B(SMat A, long dimensions) {
...
//	if(SVDVerbosity > 0) printf("START 1 svdLAS2B()\n");
/******************** Construct Asym ************************/
	int i,count,count0,col;
	double Amax=0.,alpha=1;
...
/* * * * * * * * * * * * * */
	...
	SMat Asym;
  	Asym = svdNewSMat(A->rows + A->cols, A->rows + A->cols, 2*(A->vals)+A->rows + A->cols);
  	SMat At;
// 	At = svdNewSMat(A->cols, A->rows, A->vals);
  	At = svdTransposeS(A);
	for (i=0;i<A->vals;i++)
	{
		if ( abs(A->value[i])>Amax ) Amax = abs(A->value[i]);
	}
	
//	printf("%f alpha = %f\n",rannyu(), Amax*alpha);
	
	alpha = .5*alpha*Amax + rannyu() * alpha*Amax ;
//	printf("%f alpha = %f\n",rannyu(), alpha);
	...
	for (col=0;col<At->cols;col++) {
		Asym->rowind[count] = col;
		Asym->value[count]  = alpha;
		count++;
		while (count0<At->pointr[col+1]) {
			Asym->rowind[count] = At->rowind[count0]+A->rows;
			Asym->value[count]  = At->value[count0];
			count++;
			count0++;
		}
	}
	...
	for (col=0;col<A->cols;col++) {
		while (count0<A->pointr[col+1]) {
			Asym->rowind[count] = A->rowind[count0];
			Asym->value[count]  = A->value[count0];
			count++;
			count0++;
		}
		Asym->rowind[count] = At->cols + col;
		Asym->value[count]  = alpha;
		count++;
	}
	for(i=0; i<= At->cols; i++)
		Asym->pointr[i] = At->pointr[i]+i;
	for(i=0; i<= A->cols; i++)
		Asym->pointr[At->cols+i] = At->vals+At->cols + A->pointr[i] + i;
	if (SVDVerbosity > 2) {
		printf("generating Asym : \n");
		printf("count=%i : vals=%li : Amax=%e \n",count,Asym->vals,Amax);
		printf("Asym->pointer[] : \n");
		if (SVDVerbosity > 2) {
			for(i=0; i<= Asym->cols; i++)
				printf(" %li ",Asym->pointr[i]);
			printf("\nAsym->(row,val)[] : \n");
			for (i=0;i<Asym->vals;i++) 
				printf(" (%li,%f) ",Asym->rowind[i],Asym->value[i]);
	      		printf("\nResulting Asym: \n");
	     		svdWriteSparseMatrix(Asym, "-", SVD_F_DT);
		}
	}

/******************** Run SVD on Asym ************************/
	SVDRec SVD_Asym;
	SVD_Asym = svdLAS2(Asym, 2*dimensions, 0, end, kappa);
//    		printf("\nSINGULAR VALUES: ");
 //  		svdWriteDenseArray(SVD_Asym->S, SVD_Asym->d, "-", FALSE);
  	if (SVDVerbosity > 2) {
    		printf("\nSINGULAR VALUES: ");
    		svdWriteDenseArray(SVD_Asym->S, SVD_Asym->d, "-", FALSE);
      		printf("\nLEFT SINGULAR VECTORS (transpose of U): ");
      		svdWriteDenseMatrix(SVD_Asym->Ut, "-", SVD_F_DT);
      		printf("\nRIGHT SINGULAR VECTORS (transpose of V): ");
      		svdWriteDenseMatrix(SVD_Asym->Vt, "-", SVD_F_DT);
  	} 
/****** Find the correct right and left singular vectors of A from SVDsol********** 
 * u0(:,i) = u(0:A->rows-1,i)./||u(0:A->rows-1,i)|| 
 * v0(:,i) = u(A->rows:A->rows+A->cols,i)./||u(A->rows:A->rows+A->cols,i)|| 
 * while(count<2r,count0<r)
 *   if U0[count][] is already added, count++
 *   else copy U0[count][] to Usol[count0][]
 **********************************************************************************/
	double sum=0.,max=0.;
	SVDRec SVDsol;
	SVDsol = svdNewSVDRec();
	SVDsol->d = dimensions;
	SVDsol->S = svd_doubleArray(SVDsol->d, TRUE, "svdlas2B: SVDsol->s");
	SVDsol->Ut = svdNewDMat(SVDsol->d, A->rows);
	SVDsol->Vt = svdNewDMat(SVDsol->d, A->cols);

	int j;
	int *indic;
	indic = malloc(2*dimensions*sizeof(int));

	for( i=0; i<2*dimensions; ++i)
		indic[i] = 1;
	
	int *msindic;
	msindic = malloc(dimensions*sizeof(int));

	for(i=0; i<dimensions; ++i)
	{
		msindic[i] = find_msindic(indic,SVD_Asym,alpha,A->rows);
...
/* * * * Write Singular Values and vectors * * * */
	...
	for(i=0; i<dimensions; ++i)
	{
		SVDsol->S[i] = SVD_Asym->S[msindic[i]] - alpha ;
//		printf("%f\n",SVDsol->S[i]);
	}	

	for(i=0; i<dimensions; ++i)
		for(j=0; j<A->cols; ++j)
			SVDsol->Vt->value[i][j] = sq2*SVD_Asym->Vt->value[msindic[i]][A->rows+j] ;
	
	for(i=0; i<dimensions; ++i)
		for(j=0; j<A->rows; ++j)
			SVDsol->Ut->value[i][j] = sq2*SVD_Asym->Vt->value[msindic[i]][j] ;
...
/*	count = 0;
	...
	sum = 0.;
	for (i=0;i<A->rows;i++)
		sum   += SVD_Asym->Ut->value[count][i]*SVD_Asym->Ut->value[count][i];
	for (i=0;i<A->rows;i++)
		SVDsol->Ut->value[count0][i] = SVD_Asym->Ut->value[count][i]/(double)sqrt(sum);
	sum=0.;
	for (i=0;i<A->cols;i++)
		sum   += SVD_Asym->Ut->value[count][A->rows+i]*SVD_Asym->Ut->value[count][A->rows+i];
	for (i=0;i<A->cols;i++)
		SVDsol->Vt->value[count0][i] = SVD_Asym->Ut->value[count][A->rows+i]/(double)sqrt(sum);
	SVDsol->S[count0] = SVD_Asym->S[count]-alpha*Amax;
	count++;
	count0++;
	while ( count<2*dimensions && count0<dimensions ) {
		sum = 0.;
		max = 0.;
		for (i=0;i<A->rows;i++)
			sum   += SVD_Asym->Ut->value[count][i]*SVD_Asym->Ut->value[count][i];
		for (i=0;i<A->rows;i++)
			SVDsol->Ut->value[count0][i] = SVD_Asym->Ut->value[count][i]/(double)sqrt(sum);
		for(i=0;i<count0;i++) {
			sum = svd_ddot(A->rows, SVDsol->Ut->value[i], 1, SVDsol->Ut->value[count0], 1);
			if (sum<0) sum = -1.*sum;
			if ( max<sum ) max = sum;
			if ( SVDVerbosity > 2 ) printf("count0=%i compare=%i sum=%f \n",count0,i,sum);
			printf("count0=%i compare=%i sum=%f \n",count0,i,sum);
		}
		// Check if this singular vector has already been added; or sigma is negative
		if ( max>1-0.01 || SVD_Asym->S[count]-alpha*Amax<0 ) count++; 
		else {
			sum=0.;
			for (i=0;i<A->cols;i++)
				sum   += SVD_Asym->Ut->value[count][A->rows+i]*SVD_Asym->Ut->value[count][A->rows+i];
	...
	if (SVDVerbosity > 1) {
		printf("\n[FINAL SOLUTION] SINGULAR VALUES: ");
		svdWriteDenseArray(SVDsol->S, SVDsol->d, "-", FALSE);
	}
	if (SVDVerbosity > 2) {
	        printf("\n[FINAL SOLUTION] LEFT SINGULAR VECTORS (transpose of U): ");
		svdWriteDenseMatrix(SVDsol->Ut, "-", SVD_F_DT);
		printf("\n[FINAL SOLUTION] RIGHT SINGULAR VECTORS (transpose of V): ");
		svdWriteDenseMatrix(SVDsol->Vt, "-", SVD_F_DT);
	}
*/
/* *  * * * * * * * * * * * * * * * * * * */

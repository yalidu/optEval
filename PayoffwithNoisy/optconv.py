...
if players>=150:
    interval=60
cut=players//20
mlist=[i for i in range((players*players)//cut+500,(players*players)//cut+1000+1,interval)]
...
base_params = {
    ...
    "MC":[True],
    "MCr":[5],#rlist,#[init_r],
    "MCm":mlist,#mlist,#[45,50,75,90]+[40,55,60,65,70,80,85]+[95]+[100,120,150,175,200],#[45,50,75,90,100,120,150,175,200],#[i for i in range(2000,6000+1,1000)]+[i for i in range(8000,32000+1,4000)],
    ...
    "t_max":[100*players*players],# [4000000],#[1000* 1000],  # Maximum timesteps to run for
...
exp_params = [
    ...
    {
        "sampler": ["freq2"],
        "delta": [delta],#[0.4, 0.3, 0.2, 0.1, 0.05, 0.01, 0.001],
        "repeats": [1]#[1 + i for i in range(6)],
    }
]
print("--------------")
print(env_params)
...
env_dict["bernoulli"]=partial(game.GaussianOnePopGames,actions=players)#partial(game.Bernoulli_game)
#env_dict['bad_good']=partial(game.BadAgentTies,actions=5,hardness=10)
num_in_parallel = 20  # Number of runs to launch in parallel
folder = "alphaRankRunsconv"  # Folder to save the experiments
...
def run_exp(d):
    ...
    tmax=100*num_strats*num_strats#exp["t_max"]
    ...
    RGUCBerror=np.load('./convnoisy/{}.npy'.format(players))[1]
    logging_info['conv']=0
    if logging_info['alpha_error']<=RGUCBerror:
        #np.save('./convnoisy/optm{}.npy'.format(players),exp["MCm"])
        print(players,"converge at",exp['MCm'])
        #exit()
        logging_info['conv']=exp['MCm']
...
print(direc)
date_str = "opteval{}".format(players)#"RGUCB"#"A"+str(datetime.date.today())
    ...
    with Pool(num_in_parallel) as pool:
        ...
        for exp_data in pool.map(run_exp, [(i, exp) for i, exp in enumerate(exps)]):
            ...
            if SAVING:
                ...
                print(exp_data["exp_info"]['MCm'])
                # Save to a new file
                name_num = ix
                name = "{}_{}".format(date_str, name_num)
                print(direc+name)
                if exp_data['conv']!=0:
                    convat=min(convat,exp_data['conv'])

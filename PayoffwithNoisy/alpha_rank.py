...
def alpha_rank(M, alpha=1, mutation=50, use_inf_alpha=False, inf_alpha_eps=0.01, use_sparse=False, use_cache=False):
    ...
    if strat_to_index is None or not use_cache or (
            strat_to_index is not None and (cache_strats, cache_players) != (num_strats, num_players)):
    ...
    for base_strat in product(range(num_strats), repeat=num_players):
        ...
        for n in range(num_players):
            for strat_index in range(num_strats):
                ...
                if use_inf_alpha:
                    if abs(
                            fitness_diff) < 0.0001:  # Using this for speed, np.isclose() was taking a significant chunk of time
                    ...
                    if abs(fitness_diff) < 0.0001:
                        ...
                        if -alpha * mutation * fitness_diff > 500:
                            ...
                            transition_prob = eta * (1 - np.exp(-alpha * fitness_diff)) / (
                                        1 - np.exp(-alpha * mutation * fitness_diff))
    ...
    if use_sparse:
        ...
        if num_stationary_eigenvecs != 1:
            if use_inf_alpha:
                raise ValueError(
                    "{} stationary distributions found using Sparse Solver. Using infinite alpha with pertub: {:.8f}, Mutation: {}".format(
                        num_stationary_eigenvecs, inf_alpha_eps, mutation))
            else:
                raise ValueError(
                    "{} stationary distributions found using Sparse Solver. Alpha: {:.2f}, Mutation: {}".format(
                        num_stationary_eigenvecs, alpha, mutation))
        ...
        if num_stationary_eigenvecs != 1:
            if use_inf_alpha:
                raise ValueError(
                    "{} stationary distributions found. Using infinite alpha with pertub: {:.8f}, Mutation: {}".format(
                        num_stationary_eigenvecs, inf_alpha_eps, mutation))
            else:
                raise ValueError(
                    "{} stationary distributions found. Alpha: {:.2f}, Mutation: {}".format(num_stationary_eigenvecs,
                                                                                            alpha, mutation))
        left_eigenvecs *= 1. / sum(left_eigenvecs)  # Ensuring it is a valid probability distribution
        phi = left_eigenvecs.real.flatten()

...
base_params = {
    ...
    "MC":[True],
    "MCr":[init_r],
    "MCm":[i for i in range(4000,6000+1,1000)]+[i for i in range(8000,32000+1,4000)],#[2000,2500,3000,3500,4000,4500,5000,5500,6000,7000,8000,9000,9500],#[45,50,75,90]+[40,55,60,65,70,80,85]+[95]+[100,120,150,175,200],#[45,50,75,90,100,120,150,175,200],#[i for i in range(2000,6000+1,1000)]+[i for i in range(8000,32000+1,4000)],
    ...
    "t_max":[100*200*200],# [4000000],#[1000* 1000],  # Maximum timesteps to run for
...
exp_params = [
    # ResponseGraphUCB with a range of \delta values
    {
        ...
        "repeats": [1,2]#[1 + i for i in range(6)],
    }
]
print("--------------")
print(env_params)
...
env_dict["bernoulli"]=partial(game.Soccer_game,actions=200)#partial(game.Bernoulli_game)
#env_dict['bad_good']=partial(game.BadAgentTies,actions=5,hardness=10)
num_in_parallel = 10  # Number of runs to launch in parallel
folder = "alphaRankRuns2P1"  # Folder to save the experiments
...
def run_exp(d):
    ...
    logging_info["env_info"] = {
        "true_payoffs": 2*payoffs.true_payoffs()-1,
        "num_pops": num_pops,
...
direc = "./marleval/picture_data/0.001/noisysoccer/{}".format(folder)
if SAVING and not os.path.exists(direc):
    os.makedirs(direc, exist_ok=True)
print(direc)
date_str = "optevaltr{}".format(init_r)#"RGUCB"#"A"+str(datetime.date.today())
